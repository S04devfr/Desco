...
// GET /api/dashboard/operator-daily-leads — Operatorlar kunlik leadlar bilan ishlash statistikasi
router.get('/operator-daily-leads', async (req, res) => {
  try {
    ...
    // 1. Fetch active users (managers & admins)
    const users = await prisma.user.findMany({
      where: { isActive: true },
      select: { id: true, fullName: true, email: true, role: true, avatar: true }
    });

    // 2. Fetch stage movements in the period
    const stageHistories = await prisma.dealStageHistory.findMany({
      where: {
        changedAt: { gte: start, lte: end }
      },
      include: {
        fromStage: { select: { id: true, name: true, statusType: true } },
        toStage: { select: { id: true, name: true, statusType: true } },
        deal: {
          select: {
            id: true,
            productName: true,
    ...
    // 3. Fetch activity logs in the period
    ...
    // 4. Fetch deals created in the period
    const createdDealsInPeriod = await prisma.deal.findMany({
      where: {
        createdAt: { gte: start, lte: end }
      },
      select: {
        ...
        stageId: true,
        createdAt: true,
        stage: { select: { id: true, name: true, statusType: true } }
      }
    });

    // 5. Fetch deals managed or updated in period
    const managedDealsInPeriod = await prisma.deal.findMany({
      where: {
        OR: [
          { createdAt: { gte: start, lte: end } },
          { stageUpdatedAt: { gte: start, lte: end } },
    ...
    const isPeregovor = (name) => {
      const n = (name || '').toLowerCase();
      return n.includes('peregovor') || n.includes('peregavor') || n.includes('muzokara') || n.includes('taklif');
    };

    const isCallback = (name) => {
      const n = (name || '').toLowerCase();
      return n.includes("ko'tarmadi") || n.includes('kotarmadi') || n.includes('qayta') || n.includes('aloqa') || n.includes('javob');
    };

    const isWon = (status, stageName) => {
      if (status === 'won') return true;
      const n = (stageName || '').toLowerCase();
      return n.includes('100%') || n.includes('yutil') || n.includes('olindi') || n.includes('sotildi');
    };

    const isLost = (status, stageName) => {
      if (status === 'lost') return true;
      const n = (stageName || '').toLowerCase();
      return n.includes('rad') || n.includes('otkaz') || n.includes('lost') || n.includes('negativ');
    };

    // Calculate metrics per operator
    const operatorsStats = users.map(u => {
      const uHistory = stageHistories.filter(h => h.changedById === u.id);
      const uActivities = activities.filter(a => a.userId === u.id);
      const uDealsCreated = createdDealsInPeriod.filter(d => d.managerId === u.id);
      const uDealsManaged = managedDealsInPeriod.filter(d => d.managerId === u.id);

      // Set of unique deal IDs touched/worked by this operator in this period
      const handledDealIds = new Set();
      uHistory.forEach(h => handledDealIds.add(h.dealId));
      uActivities.forEach(a => { if (a.dealId) handledDealIds.add(a.dealId); });
      uDealsCreated.forEach(d => handledDealIds.add(d.id));

      // 1. Yangi -> Peregovor (or moved into Peregovor)
      const peregovorDealIds = new Set();
      uHistory.forEach(h => {
        if (h.toStage && isPeregovor(h.toStage.name)) {
          peregovorDealIds.add(h.dealId);
        }
      });
      uDealsManaged.forEach(d => {
        if (d.stage && isPeregovor(d.stage.name) && d.stageUpdatedAt && d.stageUpdatedAt >= start && d.stageUpdatedAt <= end) {
          peregovorDealIds.add(d.id);
        }
      });

      // 2. Ko'tarmadi / Qayta aloqa
      const callbackDealIds = new Set();
      uHistory.forEach(h => {
        if (h.toStage && isCallback(h.toStage.name)) {
          callbackDealIds.add(h.dealId);
        }
      });
      uDealsManaged.forEach(d => {
        if (d.stage && isCallback(d.stage.name) && d.stageUpdatedAt && d.stageUpdatedAt >= start && d.stageUpdatedAt <= end) {
          callbackDealIds.add(d.id);
        }
      });

      // 3. Won / Sotuvga aylangan
      const wonDealIds = new Set();
      uHistory.forEach(h => {
        if (h.toStage && isWon(h.toStage.statusType, h.toStage.name)) {
          wonDealIds.add(h.dealId);
        }
      });
      uDealsManaged.forEach(d => {
        if (isWon(d.status, d.stage?.name) && d.stageUpdatedAt && d.stageUpdatedAt >= start && d.stageUpdatedAt <= end) {
          wonDealIds.add(d.id);
        }
      });

      // 4. Lost / Bekor qilingan
      const lostDealIds = new Set();
      uHistory.forEach(h => {
        if (h.toStage && isLost(h.toStage.statusType, h.toStage.name)) {
          lostDealIds.add(h.dealId);
        }
      });
      uDealsManaged.forEach(d => {
        if (isLost(d.status, d.stage?.name) && d.stageUpdatedAt && d.stageUpdatedAt >= start && d.stageUpdatedAt <= end) {
          lostDealIds.add(d.id);
        }
      });

      const totalHandled = Math.max(handledDealIds.size, peregovorDealIds.size + callbackDealIds.size + wonDealIds.size + lostDealIds.size);
      const wonCount = wonDealIds.size;
      const peregovorCount = peregovorDealIds.size;
      const callbackCount = callbackDealIds.size;
      const lostCount = lostDealIds.size;
      const conversionRate = totalHandled > 0 ? Number(((wonCount / totalHandled) * 100).toFixed(1)) : 0;
      const totalActions = uHistory.length + uActivities.length;
    ...
    // Sort operators: active managers with highest handled count first
    operatorsStats.sort((a, b) => b.totalHandled - a.totalHandled || b.wonCount - a.wonCount);

    const summary = {
      totalNewLeads: createdDealsInPeriod.length,
      totalHandled: operatorsStats.reduce((s, o) => s + o.totalHandled, 0),
      totalPeregovor: operatorsStats.reduce((s, o) => s + o.peregovorCount, 0),
      totalWon: operatorsStats.reduce((s, o) => s + o.wonCount, 0),
      totalCallback: operatorsStats.reduce((s, o) => s + o.callbackCount, 0),
      totalLost: operatorsStats.reduce((s, o) => s + o.lostCount, 0)
    };
    summary.overallConversion = summary.totalHandled > 0 ? Number(((summary.totalWon / summary.totalHandled) * 100).toFixed(1)) : 0;
    ...
    console.error('Operator daily leads error:', err);
    res.status(500).json({ message: 'Statistikani yuklashda xatolik: ' + err.message });
  }
});
